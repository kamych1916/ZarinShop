...
def check_auth_user(response: Response, request: Request):
    if (not request.cookies) or ('session_token' not in request.cookies):
        raise HTTPException(status_code=401)
    try:
        ##user = get_current_session_user(request.headers["Access-Control-Allow"])
...
@app.get("/test_token")
async def login(user_sign_in: models.userSignin, token: str):
    return {"token":token}
...
async def patch_items(patch_item: models.patch_items, response: Response):
    ...
    link_collection.update_one({"_id": items['link_color']},
                               {"$pull": {"color_link": {
                                   "id": patch_item.id,
                                   "color": items['color']}}})
    if patch_item.link_color:
        link_collection.update_one(
                                {"_id": link_collection.find_one({"color_link":
                                                                {"$elemMatch":{"id":patch_item.link_color[0]}}})['_id']},
                                {"$push": {"color_link": {
                                   "id": patch_item.id,
        ...
        try:
            id_lc = link_collection.distinct('_id')[-1]
        except:
            id_lc = 0
        new_link_color = {
            "_id": id_lc + 1,
        ...
        link_collection.insert_one(new_link_color)
    if not cur_cat:
        raise HTTPException(status_code=403)
    ...
    if not current_item:
        raise HTTPException(status_code=403)
    return models.items(id=patch_item.id,name=patch_item.name,description=patch_item.description,size_kol=patch_item.size_kol,
...
async def get_items(id:int,response: Response):
    ...
    if not items:
        raise HTTPException(status_code=403)
    response.status_code = status.HTTP_200_OK
    return models.items(id=items['_id'],name=items['name'],description=items['description'],size_kol=items['size_kol'],
...
async def search(poisk: str, response: Response):
    ...
    for post in list_of_index:
        ...
        for p in current_items:
            list_of_items.append(models.items(id=p['_id'], name=p['name'], description=p['description'],
                                              size_kol=p['size_kol'],color=p['color'],
				      image=p['image'], price=p['price'], discount=p['discount'],
                                              hit_sales=p['hit_sales'], special_offer=p['special_offer'],
                                              categories=p['categories'],
                                              link_color=link_collection.find_one({"_id":p['link_color']})['color_link']))
