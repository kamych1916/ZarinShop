...
open class ImageSlideshow: UIView {
    ...
    func layoutScrollView() {
        ...
        for (index, view) in slideshowItems.enumerated() {
            ...
            if style == .view {
                viewHeight = view.imageView.intrinsicContentSize.height
            } else {
                viewHeight = scrollView.frame.size.height
            }
            view.frame = CGRect(x: scrollView.frame.size.width * CGFloat(index), y: 0, width: scrollView.frame.size.width, height: viewHeight)
        }
